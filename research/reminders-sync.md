...
Reminders 是任務列表的 source of truth。系統要和 macOS Reminders 中的 `待定 + 四象限` 列表完全對齊。

## 同步範圍

只同步以下五類列表名稱或其粵語/中文別名：
...
`待定 + 四象限` 外的 Reminder list 不同步入系統。
...
注意：即使設定了 list name，目前同步邏輯仍應只讀 `待定 + 四象限` 列表，避免誤同步其他 Reminders 清單。
